...
using C = ABI.API.Structure.Application.Commands.Level;
...
namespace ABI.API.Structure.Controllers.Tests
{
    ...
    public class LevelControllerTests
    {
        ...
        {
            var loggerMock = new Mock<ILogger<LevelController>>();
            var mediatrMock = new Mock<IMediator>();
            mediatrMock
                .Setup(s => s.Send(It.Is<Q.Level.GetByIdQuery>(p => p.Id.Equals(1)), default))
                .ReturnsAsync(new Application.DTO.LevelDTO());
            mediatrMock
            ...
            _controller = new LevelController(loggerMock.Object, mediatrMock.Object);
        }
        ...
        public async Task CreatTest()
        {
            var throws = await Assert.ThrowsExceptionAsync<ArgumentNullException>(() =>
                Task.Run(() => new LevelController(null, null))
            );
