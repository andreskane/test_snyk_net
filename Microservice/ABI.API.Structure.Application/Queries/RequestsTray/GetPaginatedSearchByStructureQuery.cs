...
{
    public class GetPaginatedSearchByStructureQuery : IRequest<PaginatedList<RequestTrayDTO>>
    {
        [DataMember]
    ...
    public class GetPaginatedSearchByStructureHandler : IRequestHandler<GetPaginatedSearchByStructureQuery, PaginatedList<RequestTrayDTO>>
    {
        ...
        public GetPaginatedSearchByStructureHandler(
        ...
        public async Task<PaginatedList<RequestTrayDTO>> Handle(GetPaginatedSearchByStructureQuery request, CancellationToken cancellationToken)
        {
            //Cambios en la estructura
            var changeTrackingResults = await _changeTrackingRepository.GetByStructureId(request.structureId);

            //Estructura
            var structure = await _structureRepository.GetAsync(request.structureId);

            var query = (from str in changeTrackingResults
                         where str.IdStructure == request.structureId &&
                         (str.ValidityFrom >= request.validityFrom && str.ValidityFrom <= request.validityTo)
                         select new
                         {
